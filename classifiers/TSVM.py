...
from kernels import kernel_functions as kf
from .twin_plane_1 import twin_plane_1
from .twin_plane_2 import twin_plane_2
        ...
        if(self.fuzzy==1):
            if(self.kernel_type==0):
                ...
                for i in range(len(A)):
                    term1 = kf.kernel_function(self.kernel_type,A[i],A[i],self.kernel_param)
                    term2 = 0
                    for j in range(len(A)):
                        term2 += kf.kernel_function(self.kernel_type,A[j],A[i],self.kernel_param)
                        termtemp1 += kf.kernel_function(self.kernel_type,A[i],A[j],self.kernel_param)
                    term2 = -2*term2/len(A)
                ...
                for i in range(len(B)):
                    term1 = kf.kernel_function(self.kernel_type,B[i],B[i],self.kernel_param)
                    term2 = 0
                    for j in range(len(B)):
                        term2 += kf.kernel_function(self.kernel_type,B[j],B[i],self.kernel_param)
                        termtemp2 += kf.kernel_function(self.kernel_type,B[i],B[j],self.kernel_param)
                    term2 = -2*term2/len(B)
        ...
        if(self.kernel_type==0): # no need to cal kernel
            ...
            for i in range(A.shape[0]):
                for j in range(Total_Data.shape[0]):
                    S[i][j] = kf.kernel_function(self.kernel_type,A[i],Total_Data[j],self.kernel_param)
            S = np.hstack((S,-e1))
            R = np.zeros((B.shape[0],Total_Data.shape[0]))
            for i in range(B.shape[0]):
                for j in range(Total_Data.shape[0]):
                    R[i][j] = kf.kernel_function(self.kernel_type,B[i],Total_Data[j],self.kernel_param)
            R = np.hstack((R,-e2))
        ...
        if(self.kernel_type==0): # no need to cal kernel
            ...
            for i in range(self.data_.shape[0]):
                for j in range(self.data_.shape[0]):
                    S[i][j] = kf.kernel_function(self.kernel_type,self.data_[i],self.data_[j],self.kernel_param)
            ...
            for i in range(X.shape[0]):
                for j in range(self.data_.shape[0]):
                    S[i][j] = kf.kernel_function(self.kernel_type,X[i],self.data_[j],self.kernel_param)
...
#   1-_/(|di^2|/(r-^2))    
        ...
        if(self.fuzzy==1):
            ...
            if(self.kernel_type==0): # no need to cal kernel
                ...
                for i in range(len(X)):
                    dsquaredpos = kf.kernel_function(self.kernel_type,X[i],X[i],self.kernel_param)
                    term1 = 0
                    for j in range(len(self.A_)):
                        term1 += kf.kernel_function(self.kernel_type,self.A_[j],X[i],self.kernel_param)
                    term1 = -2*term1/len(self.A_)
                    dsquaredpos += term1
                    dsquaredpos += self.termtemp1_
                    s1.append(1-np.sqrt(dsquaredpos/self.rcenpossq_))
                    dsquaredneg = kf.kernel_function(self.kernel_type,X[i],X[i],self.kernel_param)
                    term1 = 0
                    for j in range(len(self.B_)):
                        term1 += kf.kernel_function(self.kernel_type,self.B_[j],X[i],self.kernel_param)
